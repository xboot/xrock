...
Rockchip SOCs supported by XROCK and the DDR/usbplug files plus RC4 config required for maskrom initialization.

## Supported chips

| Chip | USB PID | Architecture | RC4 |
|------|---------|--------------|-----|
...
:::note
The full USB PID list is in `99-xrock.rules` at the repository root. RK3566 and RK3568 share the same PID (0x350a).
:::

## Initialization config reference

Example maskrom init commands for each chip (DDR/usbplug filenames depend on the actual version; obtain them from [rkbin](https://github.com/rockchip-linux/rkbin)).
